<template>
  <div class="permissions fluid-container mx-5" v-if="checkPermission('PERMISSION_VIEW')">
    <div class="permissions-wrapper position-relative" v-if="!isFetching">
      <!-- Title -->
      <div class="permissions-title my-4 d-flex align-items-center">
        ...
        <div class="card-body">
          ...
          <div class="row mb-3">
            <div class="col-12 text-end">
              <button @click="getPermissions" class="btn btn-success">
      ...
      <div class="card mb-3">
        <div class="card-header d-flex justify-content-between">
          <h5 class="my-2 d-flex align-items-center">Permissions</h5>
        </div>
        <div class="card-body">
          <!-- <div class="permissions-action d-flex justify-content-between mb-3">
            <div></div>
            <router-link :to="{ name: 'CreatePermission' }">
              <button class="btn btn-main btn-primary">+ New Permission</button>
            </router-link>
          </div> -->
              ...
              <tr v-for="(permission, index) in permissions" :key="permission.id">
                ...
                <!-- <td>
                  <div class="d-flex icon">
                    <router-link 
                      :to="{ name: 'ModifyPermission', params: { id: `${permission.id}` } }"
    ...
    <div class="permissions-wrapper" v-else>
  ...
  <div class="permissions fluid-container mx-5" v-else>
    <Unauthorized />
  </div>
</template>
...
import { PermissionSortField } from '@/enums/permission.enum.js'
...
export default {
  ...
  data() {
    return {
      ...
      sortCriteria: [ { sortBy: '', sortDir: '' } ], // Using sortCriteria
      checkall: false,
      checkedItems: [],
      page: 1,
      totalPages: 0,
      isFetching: true,
      alert: false,
      sortFieldOptions: PermissionSortField.toArray(),
  ...
  methods: {
    ...
    handleCheckAll() {  
      this.checkedItems = this.permissions.map(() => this.checkall)
    },
    // async handleDelete(id) {
    //   try{
    //     if (this.checkPermission('PERMISSION_DELETE')) {
    ...
    //         await permissionService.deleteOne(id)
    //         this.getPermissions()
    ...
    //   }
    // },
    // Restore filter from URL when loading page
    initFilterFromUrl() {
...
@media (max-width: 767.98px) {
  .d-flex:not(.permissions-title, .card-header) {
